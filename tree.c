...
static int tree_add_entry(Tree *tree, uint32_t mode, const ObjectID *hash, const char *name) {
    ...
    if (tree_contains_name(tree, name)) return -1;
...
static int write_tree_level(const Index *index, const char *prefix, ObjectID *id_out) {
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        if (!slash) {
            if (tree_add_entry(&tree, entry->mode, &entry->hash, rest) != 0) {
                return -1;
            }
            continue;
        }
        ...
        if (dirname_len == 0 || dirname_len >= sizeof(tree.entries[0].name)) {
            return -1;
        }
        ...
        int written = snprintf(child_prefix, sizeof(child_prefix), "%s%s/", prefix, dirname);
        if (written < 0 || (size_t)written >= sizeof(child_prefix)) return -1;

        ObjectID child_id;
        if (write_tree_level(index, child_prefix, &child_id) != 0) return -1;
        if (tree_add_entry(&tree, MODE_DIR, &child_id, dirname) != 0) return -1;
    }
