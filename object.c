...
#include <direct.h>
#include <io.h>
#define PES_MKDIR(path, mode) _mkdir(path)
#define PES_FSYNC(fd) _commit(fd)
#else
#define O_BINARY 0
#define PES_MKDIR(path, mode) mkdir((path), (mode))
#define PES_FSYNC(fd) fsync(fd)
#endif
...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    char header[64];
    int header_len_i = snprintf(header, sizeof(header), "%s %zu", type_name, len);
    if (header_len_i < 0 || (size_t)header_len_i >= sizeof(header)) return -1;

    size_t header_len = (size_t)header_len_i + 1; // include terminating '\0'
    ...
    *id_out = id;
    ...
    if (PES_MKDIR(shard_dir, 0755) != 0 && errno != EEXIST) {
        free(full_object);
        return -1;
    }
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    ObjectID actual_id;
    compute_hash(file_data, file_size, &actual_id);
    if (memcmp(actual_id.hash, id->hash, HASH_SIZE) != 0) {
        free(file_data);
        return -1;
    }

    uint8_t *nul = memchr(file_data, '\0', file_size);
    if (!nul) {
        free(file_data);
        return -1;
    }
    ...
    size_t data_len;
    char extra;
    if (sscanf(header, "%15s %zu%c", type_name, &data_len, &extra) != 2) {
        free(file_data);
        return -1;
    }
    ...
    if (data_offset > file_size || data_len != file_size - data_offset) {
        free(file_data);
        return -1;
    }
    ...
    if (data_len > 0) memcpy(data, file_data + data_offset, data_len);
