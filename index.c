...
#define PES_INDEX_MODE(st_mode) 0100644
#else
#define PES_FSYNC(fd) fsync(fd)
#define PES_INDEX_MODE(st_mode) (S_IXUSR & (st_mode) ? 0100755 : 0100644)
#endif
...
int index_load(Index *index) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (parsed != 5 || strlen(hash_hex) != HASH_HEX_SIZE) {
            fclose(f);
            return -1;
        }
...
int index_save(const Index *index) {
    ...
    if (index->count > 0) {
        ...
        qsort(sorted, (size_t)index->count, sizeof(IndexEntry), index_entry_cmp);
    }
    ...
    if (fflush(f) != 0 || PES_FSYNC(fileno(f)) != 0 || fclose(f) != 0) {
        free(sorted);
        unlink(tmp_path);
        return -1;
    }
...
int index_add(Index *index, const char *path) {
    ...
    memset(entry, 0, sizeof(*entry));
    entry->mode = PES_INDEX_MODE(st.st_mode);
    entry->hash = blob_id;
